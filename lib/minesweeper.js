...
let notMineArr = [];
let gridMineObjArr = []
let numOfMines = Math.floor(width * width / 7);
let gridArr = [];

const createGrid = () => {
  const wholeGrid = document.querySelectorAll('tr');
  wholeGrid.forEach(row => row.remove())
  while (gridMineObjArr.length > 0) {
    gridMineObjArr.pop()
  }
...
const newGridSetup = () => {
  ...
  gridArr.forEach((cell, index) => {
    ...
    return gridMineObjArr.push(cellObj)
  });
...
const emptyCellIsland = (index) => {
  ...
  // check bottom right
  if (id < gridArr.length - 1 && !isRightEdge) {
    const newId = id + width + 1
    if (gridMineObjArr[newId]) {
      exposeCell(gridMineObjArr[newId])
    }
  }
...
document.getElementById("newGame").addEventListener("click", () => {
  ...
  emptyOutArrays(minesArr)
  emptyOutArrays(notMineArr)
  emptyOutArrays(gridMineObjArr)
  emptyOutArrays(gridArr)
  createGrid();
  // newGridSetup();
  // newGameStart();
})
